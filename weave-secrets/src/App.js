...
function App() {
  ...
  const [bundler, setBundler] = React.useState();
  const [bundlerHttpAddress, setBundlerAddress] = React.useState(
    "https://node1.bundlr.network"
  );
  const [fundAmount, setFundingAmount] = React.useState();
  ...
  const providerMap = {
    ...
    "WalletConnect": async (c) => { return await connectWeb3(await (new WalletConnectProvider(c)).enable()) },
    "Phantom": async (c: ) => {
  ...
  const initProvider = async () => {
    ...
    const providerInstance = await p(c.opts).catch((e) => { toast({ status: "error", title: `Failed to load provider ${pname}`, duration: 10000 }); console.log(e); return; })
  ...
  const toProperCase = (s) => { return s.charAt(0).toUpperCase() + s.substring(1).toLowerCase() }
